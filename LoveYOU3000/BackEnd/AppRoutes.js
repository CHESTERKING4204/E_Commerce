...
route.delete('/cart/:id', async (request, response) => {
    try {
      const id = request.params.id;
      console.log("Deleting item with ID:", id); // Log the id being passed in
      ...
      console.log("Deleted item:", del); // Log the deleted item
      response.status(200).json(del); // Return the deleted item
    } catch (error) {
...
route.get('/product/search', async (req, res) => {
    try {
      ...
      const products = await Product.find({ tags: { $regex: tag, $options: 'i' } });
